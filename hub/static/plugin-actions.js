...
function handlePluginActions(event) {
  ...
  if(pluginDeploy){
    ...
    $('#plugin-iran-node').value=iran.id;if(kharej){$('#plugin-kharej-node').value=kharej.id;form.elements.kharej_endpoint.value=kharej.host;}$('#plugin-endpoint').value=plugin?.ui?.endpoint_default_side==='kharej'?(kharej?.host||''):iran.host;updatePluginMode();openModal('#plugin-modal');return true;
  }
  ...
  if(pairReveal){api(`/api/hybrid-deployments/${Number(pairReveal.dataset.deploymentId)}/pair-code`,{method:'POST'}).then(result=>{const name=result.plugin_name||'matching DARK tunnel plugin';$('#output-title').textContent='DARK NOC PAIR CODE';$('#job-output').textContent=`KEEP THIS CODE SECRET\n\n${result.pair_code}\n\nOn the KHAREJ server run ${name}, select KHAREJ, choose Connect with DARK NOC Pair Code, and paste this code.`;openModal('#output-modal');}).catch(error=>showToast('PAIR CODE FAILED',error.message,true));return true;}
