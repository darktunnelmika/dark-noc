...
function renderLiveTopology() {
  ...
  groups.forEach(rows=>{
    ...
    const peerId=Number(representative.peer_node_id||0),peerNode=nodeIndex.get(peerId);
    ...
    if(left.node&&left.node.status!=='online')statuses.push('stale');if(right.node&&right.node.status!=='online')statuses.push('stale');
    ...
    if(state.topologyFilter!=='all'&&state.topologyFilter!==tone)return;if(state.topologySearch&&!haystack.includes(state.topologySearch))return;
