...
def reconfigure_tunnel_mutation(
    ...
    *,
    ...
    with db() as conn:
        ...
        if not token:
            raise NodeTunnelServiceError(409, "This deployment has no recoverable pair token")
        jobs: dict[str, int] = {}
        roles = plugin["roles"]
        if managed:
...
WRAPPERS: dict[str, str] = {
    "create_node": '''enrollment = secrets.token_urlsafe(36)
try:
...
return {"id": node_id, "name": body.name, "provisioning": True, "message": "Secure SSH installation started"}''',
    "retry_node_provision": '''enrollment = secrets.token_urlsafe(36)
try:
...
return result''',
    "remove_tunnel_from_manager": '''try:
...
def replace_function_body(text: str, name: str, body: str) -> str:
    ...
    if not node.body or node.end_lineno is None:
        raise AssertionError(f"Could not resolve function body: {name}")
    ...
    if not indented.endswith("\n"):
        indented += "\n"
    return "".join(lines[:start] + [indented] + lines[end:])


text = APP_PATH.read_text()
for function_name, body in WRAPPERS.items():
    text = replace_function_body(text, function_name, body)

anchor = "del _monitor_incident_repository_name\n"
...
Path("hub/node_tunnel_service.py").write_text(SERVICE)
...
'''
Path("tests/backend_node_tunnel_service_contract.py").write_text(contract)
...
for path in identity_paths:
    ...
    if "2.9.24" in value:
        path.write_text(value.replace("2.9.24", "2.9.25"))

notes = '''# DARK NOC v2.9.25 — Node & Tunnel Mutation Service Layer
