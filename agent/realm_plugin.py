...
def _port_listening(port: int) -> bool:
    ...
    if code != 0:
        return False
    # ss -H -lnt: State Recv-Q Send-Q Local-Address:Port Peer-Address:Port.
    # Searching the whole row can mistake an IPv6 segment for a TCP port.
    expected = str(port)
    ...
    try:
        for port in ports:
            ...
            if "Rule added" in output:
                created.append(rule)
        # A redeploy often skips existing rules. Keep their ownership for remove(),
        # but return only new rules so a failed redeploy cannot delete old ones.
        owned = list(dict.fromkeys([*previous, *created]))
        _atomic_text(ownership_path, json.dumps(owned), 0o600)
    except Exception:
        # This also covers a failed metadata write after opening new ports.
